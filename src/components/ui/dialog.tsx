...
>(({ className, children, showClose = true, disableAnimations = false, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay
      className={disableAnimations ? 'data-[state=open]:animate-none data-[state=closed]:animate-none' : undefined}
    />
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-xl bg-card p-6 shadow-[var(--elevation-lg)] outline-none focus:outline-none focus-visible:outline-none duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out',
...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-col-reverse gap-3 sm:flex-row sm:justify-end sm:space-x-2', className)} {...props} />
)
