...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'USAGE'
Usage:
  ./scripts/deploy-r2-voice-comments.sh [--skip-secrets]
...
echo "Deploying function: r2-voice-comments"
supabase functions deploy r2-voice-comments --project-ref "$PROJECT_REF" --no-verify-jwt
